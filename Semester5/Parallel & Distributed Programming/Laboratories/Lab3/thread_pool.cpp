...
int main()
{
    ...
    // Choose the split method here and launch tasks using a thread pool managed by std::async
    // For row-wise split
    for (int i = 0; i < numberOfTasks; ++i)
    {
        futures.emplace_back(std::async(std::launch::async, computeTaskRow, std::ref(A), std::ref(B), std::ref(Result), i, numberOfTasks));
    }

    // For column-wise split (comment out the row-wise section and uncomment this section)
    /* for (int i = 0; i < numberOfTasks; ++i)
    {
        futures.emplace_back(std::async(std::launch::async, computeTaskColumn, std::ref(A), std::ref(B), std::ref(Result), i, numberOfTasks));
    } */

    // For stride split (comment out the above sections and uncomment this section)
    /* for (int i = 0; i < numberOfTasks; ++i)
    {
        futures.emplace_back(std::async(std::launch::async, computeTaskStride, std::ref(A), std::ref(B), std::ref(Result), i, numberOfTasks));
    } */
